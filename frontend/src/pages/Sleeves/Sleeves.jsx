...
const colorCounts = useMemo(() => {
  ...
  APIproducts?.forEach(item => {
    if (item.category === "sleeves" && Array.isArray(item.colors)) {
  ...
  const filteredProducts = useMemo(() => {
    let filtered = APIproducts?.filter(p => p.category === "sleeves") || [];
  ...
  return (
    <div className="p-0">
      <div className="bg-[#0000001d] border-b flex items-center justify-center px-2 md:px-6 py-6 text-center">
        <h1 className="md:text-2xl text-2xl font-semibold">Laptop Sleeves</h1>
      </div>
          ...
          {filteredProducts.length === 0 ? (
            ...
            filteredProducts.slice(0, visibleProducts).map((item, index) => (
              <div key={item._id}>
                ...
                />
                {/* Product Name show karwana ho to yahan bhi likh sakte hain */}
                {/* <div className="text-center text-sm font-semibold mt-1">{item.productName}</div> */}
              </div>
            ))
          )}
        </div>
